...
{
	IDOS->SetVar("uuid/store", (CONST_STRPTR) &store, sizeof(struct uuid_store),
                 (unsigned long)(GVF_GLOBAL_ONLY | GVF_SAVE_VAR | GVF_BINARY_VAR));
}
...
UBYTE *store_get_mac(void)
{
	...
	return (UBYTE *) &store.mac;
}

static void store_load(void)
{
	if(IDOS->GetVar("uuid/store", (STRPTR) &store, sizeof(struct uuid_store) - 1,
