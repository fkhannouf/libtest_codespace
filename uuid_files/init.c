...
if (!(IExec->CheckIO((struct IORequest *) libBase->TimerIO)))
{
    IExec->AbortIO((struct IORequest *)libBase->TimerIO);      /* Ask device to abort any pending requests */
}
 
IExec->WaitIO((struct IORequest *)libBase->TimerIO);          /* Clean up */
...
STATIC struct Library *libInit(struct Library *LibraryBase, APTR seglist, struct Interface *exec)
{
    ...
    libBase->segList = (BPTR)seglist;
       ...
       UtilityBase = (struct UtilityBase *) IExec->OpenLibrary("utility.library", 50L);
       if (UtilityBase)
       {
           IUtility = (struct UtilityIFace *)IExec->GetInterface(
	...
	if(IExec->OpenDevice(TIMERNAME, UNIT_MICROHZ, (struct IORequest *) libBase->TimerIO, 0))
		return NULL;
